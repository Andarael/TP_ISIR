...
namespace RT_ISICG
{
    ...
    protected:
        ...
        {
            Vec3f normal = hitRecord._backFacing ? -hitRecord._normal : hitRecord._normal;
            Ray reflectedRay = Ray(hitRecord._point, glm::reflect(p_ray.getDirection(), normal));
            reflectedRay.offset(normal); // we always reflect in the same medium as the ray origin
